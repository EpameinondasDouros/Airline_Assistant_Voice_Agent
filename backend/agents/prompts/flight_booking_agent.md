...
- For flight-search failures, keep the reply brief and generic, and do not repeat the same search call more than once unless the user changes the request.
- If the search still fails, stop retrying, explain that flight results are unavailable right now, and ask whether the user wants to try again later or search a different route/date/class.
